...
- **`.claude/skills/build-app/`** — gradi NOV alat od nule isključivo pričanjem sa korisnikom (suprotno od `install-app` koji prima već gotov kod). Pretvara opis ("napravi mi widget koji...") u kompletan `apps/<id>/` folder (manifest, widget/app/oboje, `server_ext.py` po potrebi, verzija uvijek `1.0.0`), registruje ga u `index.html` - alat se odmah pojavi u katalogu, korisnik NE mora prije toga praviti sekciju. Prije "gotovo" prolazi obaveznu prenosivost provjeru (izolovan test kopije foldera) - cilj je da se rezultat može podijeliti i instalirati na BILO KOM drugom AI Zdravo Dashboardu bez izmjene. Koristi se kad korisnik opiše šta alat treba da radi bez da da gotov kod. Isti Claude/Codex dvostruki-fajl obrazac kao install-app/update-dashboard.
